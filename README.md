...
- 插件版本：`0.3.0-selfhosted.21+codex.20260825113550`（以 `plugins/mem0/.codex-plugin/plugin.json` 为唯一版本源）
...
| 层级 | 已提供能力 |
| --- | --- |
| MCP | 14 个工具，覆盖私有项目范围解析、风险分级写入、当前/历史时间点搜索、分页读取、详情、更新、单条删除、历史、实体枚举、候选审核、版本化反馈和两阶段批量管理 |
...
| 自动记忆 | 新仓库跨机器范围自动解析、证据与置信度分级、候选隔离、精确去重与 LLM 冲突分类、时间有效性、确认别名扩展、智能多查询检索、真实 rerank、版本化反馈、Memory Decay、分类保留和压缩前后摘要 |
| Dashboard 与治理 | 签名游标分页、候选批量审核、冲突工作台、检索逐阶段解释、历史恢复、版本化质量审查、有效使用事件、画像增量投影和可逐提案审批、完整回滚/重试的 Dream 整理 |
| 平台运维 | 后台任务、请求配额、定时与手动备份、预览恢复、JSON/CSV 导入、评测 v2 与失败回放、审计导出、可选 S3 灾备/恢复演练、完整监控栈、用户和项目权限 |
| 安全 | 管理员 REST Key 与 MCP Key 用途隔离，`admin`/`editor`/`viewer` 角色控制，强制项目边界，治理写操作预览确认，批量删除默认关闭 |

升级前已有的 11 个 MCP 工具保持旧参数和基础响应兼容；当前插件在此基础上新增 3 个候选审核与反馈工具，未升级客户端仍可继续使用基础读写能力。
...
| 模块 | 能力与安全边界 |
| --- | --- |
| 检索工作台 | 使用自然语言执行混合、向量或关键词检索，支持 `current` 与 `as_of` 时间有效性，按项目、User、Agent、Run、时间、类型、分类和 metadata 筛选；使用项目内已确认别名扩展查询，并显示 `retrieval_id`、`ranking_trace_id`、各阶段排名、过滤原因、语义/RRF/rerank/反馈/置信度/Decay/最终分数和明确错误状态 |
| 候选、冲突、历史与质量 | 审核自动提取的候选记忆，支持确认、拒绝、恢复 pending 或编辑后重评；非精确相似事实进入带缓存与失败降级的 LLM 冲突队列，可并排处理重复、替代、矛盾、条件并存和不相关结论；查看完整版本快照并恢复旧版本；反馈绑定 `memory_id + payload_version + actor`，旧版本反馈不会污染新版本 |
| 批量维护 | 所有治理列表支持带筛选哈希的 HMAC 签名游标和稳定分页；候选批量审核、正文、metadata、过期时间和项目范围变更均采用“预览 → 哈希确认 → 版本校验 → 执行”，单次最多 500 条，预览后发生变化会拒绝执行 |
| 活动与排序 | 按操作、记忆、成功状态和来源查看活动记录；曝光与打开、采用、复制、反馈分开记录，只有有效使用会增加热度；反馈因子和有频率上限的 Decay 共同影响最终排序 |
| Dream 与画像 | Dream 由后端调度并按项目生成提案，每条提案可单独接受、拒绝或编辑，执行前展示影响记忆、版本和关系差异；旧记忆只标记为 `superseded`，不物理删除；支持条件并存、待确认、完整回滚和部分失败后的逐项重试，敏感或受保护记忆强制人工审批；画像、实体和别名通过 outbox 增量刷新，仍可全量重建且不改写原始事实 |
| 平台运维 | 查看和取消待执行后台任务，创建与恢复最多 100000 条记忆的备份，预览 JSON/CSV 导入；评测 v2 支持从检索会话创建用例、失败回放、逐例排名轨迹、Recall@K、Precision@K、MRR、NDCG、项目聚合、趋势和基线发布门禁；活动 CSV 通过后台任务导出且默认不含记忆正文 |
...
请求配额默认按“认证主体 + 项目”统计：搜索每分钟 60 次并允许 20 次突发，写入每分钟 20 次并允许 10 次突发，批量治理每分钟 2 次，导出和评测等重任务每小时 2 次。超限返回 `429` 与 `Retry-After`；只有幂等读取和带 operation ID 的可重试操作才应退避重试。管理员可在 Dashboard 调整或关闭配额，相关使用量由 `quota_usage` 记录。
...
- PostgreSQL：记忆目录、候选、关系、冲突判定、Mutation、投影 outbox、检索会话与逐阶段事件、版本化反馈与聚合、画像投影、评测 v2、用户、API Key 哈希、活动记录、治理操作、后台任务、项目权限、配额使用、灾备运行状态、MCP 删除操作和业务状态。
...
平台 JSON 备份不包含用户、API Key、质量反馈、活动记录、后台任务、评测用例或项目权限，不能替代 PostgreSQL 一致性备份，也不能单独用于完整灾难恢复。选配的生产灾备使用 Restic 客户端加密，同时保存 `mem0_app` 控制面库、`postgres` pgvector 库、History SQLite 一致性快照、运行配置、Secret 和发布清单；平台数据标记为 `mem0-dr-v3`，完整灾备清单标记为 `mem0-disaster-recovery-v3`，并记录 Git SHA、生产补丁 SHA、API/MCP/Dashboard 镜像 digest 与逐文件哈希。默认保留 7 个日备份、4 个周备份、12 个每月备份。启用恢复演练后，会按 Dashboard 设置的天数在隔离 PostgreSQL 中验证最新快照，并对账记忆数量、正文哈希、项目范围、历史、向量 collection 与孤儿引用；运行结果写入 `disaster_recovery_runs`。
