...
1. 使用 `page/page_size=200` 分页调用 `get_memories(project_id=<当前项目>)`；出现 `partial=true` 时说明本次审查不完整。
...
6. 合并时用 `update_memory(memory_id=<保留项>, project_id=<项目>, text=<合并正文>, metadata=<保留原字段后的 metadata>)`；不得修改身份、所有者、范围或 `source` 等保留字段。
