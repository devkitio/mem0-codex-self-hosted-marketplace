...
4. 调用 `update_memory(memory_id=<ID>, project_id=<项目>, text=<新正文>, metadata=<保留原字段后的 metadata>)`。如果服务未返回 metadata，只更新正文，避免覆盖未知字段。
5. 不得传入 `user_id`、`mcp_owner`、`scope`、`source` 等保留字段。
