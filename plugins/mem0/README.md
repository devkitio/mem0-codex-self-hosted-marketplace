...
- 项目范围：自动使用当前 Git 仓库根目录名，并在本机按只读 Git 远端身份阻止同名仓库静默共享；跨机器或多个克隆共享时使用显式 `project_id` 映射

脚本从插件自己的 `.mcp.json` 读取地址，不依赖用户级 `config.toml`，也不会请求官方 `api.mem0.ai`。

自动识别是本机便利策略，不是跨机器身份协议。同一台机器首次出现的目录名继续使用旧 ID，以保持现有记忆可见；同名但 Git 远端身份不同的后续仓库会得到带哈希的隔离 ID，并在启动上下文中提示。插件不会猜测旧目录名范围里的记忆属于哪个仓库，也不会自动迁移或删除；跨机器或多克隆场景应通过 `switch-project` 在每个副本上设置同一个明确 ID。
