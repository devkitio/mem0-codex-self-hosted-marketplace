...
首次启动还会自动导入 `CLAUDE.md`、`AGENTS.md`、`.cursorrules`、`.windsurfrules` 和 `mem0.md`。导入状态保存在插件数据目录，通过来源文件与 SHA-256 跳过未变化内容；短章节会在分块上限内合并，写入前先记录待完成状态。项目资料更新时，上一份有效版本会保留到新分块写入、验证和旧分块清理全部成功；失败或中断后会在下次启动续跑。项目资料被清空或删除时，只清理带有插件精确标记的旧分块；不同项目的并发状态写入会合并保存。
